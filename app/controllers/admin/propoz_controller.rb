class Admin::PropozController < AdminController
...
def index
    ...
    @propozs = Propoz.paginate(page: params[:page], per_page: per_page).order('created_at DESC')
end    

  def new
    @propoz = Propoz.new(enable: true)
  end

  def create
    @propoz = Propoz.new(propoz_params)
    if @propoz.save
      flash[:success] = 'Створено новий запис'
      file_upload @propoz.id
